...
def handler(event, context):
    ...
    new_items = [i for i in all_items if i.item_id not in existing_ids]
    near_dup_ids = find_near_duplicate_ids(new_items)
    if near_dup_ids:
        logger.info(json.dumps({"action": "near_duplicates_removed", "count": len(near_dup_ids)}))
    new_items = [i for i in new_items if i.item_id not in near_dup_ids][:MAX_ITEMS]
